...
class FlowFrameworkTests {
    ...
    fun `receiving unexpected session end before entering sendAndReceive`() {
        ...
        val sessionEndReceived = receivedSessionMessagesObservable().filter { it.message is SessionEnd }.toFuture()
    ...
    private class WaitForOtherSideEndBeforeSendAndReceive(val otherParty: Party,
                                                          @Transient val receivedOtherFlowEnd: CordaFuture<*>) : FlowLogic<Unit>() {
        @Suspendable
        override fun call() {
            // Kick off the flow on the other side ...
            send(otherParty, 1)
            // ... then pause this one until it's received the session-end message from the other side
            val thisFiber = Fiber.currentStrand()
            receivedOtherFlowEnd.then { Fiber.unpark(thisFiber) }
            Fiber.park()
